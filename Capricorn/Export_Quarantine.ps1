...
Get-QuarantineMessage -RecipientAddress $user -StartReceivedDate (Get-Date).AddDays(-9) | select Identity,SenderAddress,Subject,ReleaseStatus,ReceivedTime,@{Name="Release";Expression=" "} |Export-Csv "c:\temp\quarantined_$date.csv" -NoTypeInformation
