...
Get-QuarantineMessage -RecipientAddress $user -StartReceivedDate (Get-Date).AddDays(-3) | select Identity,SenderAddress,Subject,ReceivedTime,@{Name="Release";Expression=" "} |Export-Csv "c:\temp\quarantined_$date.csv" -NoTypeInformation
