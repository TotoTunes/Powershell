...
ForEach ($Item in $Computers){
  ...
  $targetOU = "OU=DisabledComputers1Year,OU=AD-Sanitize,DC=HBE,DC=local"
  ...
  Get-ADComputer -Filter { DistinguishedName -eq $DistName } | Move-ADObject -TargetPath $targetOU
}
