...
# Description: 
#	Will check if the BIOS is up to date, if not it will update the BIOS.
#
...
if ( $pcModel -eq "Latitude 5290 2-in-1" -and (-not($BiosVersion -eq '1.11.2')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\Dell Latitude 5290 2-in-1 1.11.2\Latitude_5290_2In1_1.11.2.exe" -ArgumentList '-r -s' -Wait
	
	# Restart Computer
	$Prompt = New-Object -comobject wscript.shell
	$Answer = $Prompt.Popup("WAARSCHUWING: De computer zal binnen 60 seconden herstarten.", 60, "BIOS Update Restart Prompt", 4096 + 48 + 1)
	Switch ($Answer) {
...
if ( $pcModel -eq "Latitude 5300 2-in-1" -and (-not($BiosVersion -eq '1.8.1')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\Dell Latitude 5300 2-in-1 1.8.1\Latitude_5300_1.8.1.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "Latitude 5501" -and (-not($BiosVersion -eq '1.8.4')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\Dell Latitude 5501 1.8.4\Latitude_5X01_Precision_3541_1.8.4.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "Latitude E7250" -and (-not($BiosVersion -eq 'A23')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\Dell Latitude E7250 A23\E7250A23.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "Latitude 5590" -and (-not($BiosVersion -eq '1.12.1')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\Dell Latitute 5590 1.12.1\Latitude_5X90_1.12.1.exe" -ArgumentList '/s /f' -Wait
...
if ( $pcModel -eq "Latitude 5591" -and (-not($BiosVersion -eq '1.11.1')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\Dell Latitute 5591 1.11.1\Latitude_5X91_Precision_3530_1.11.1.exe" -ArgumentList '/s /f' -Wait
...
if ( $pcModel -eq "HP Elite x2 1013 G3" -and (-not($BiosVersion -eq '01.10.01')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP Elite x2 1013 G3 01.10.01 Rev.A\HpFirmwareUpdRec64.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "HP EliteDesk 800 G2 DM 65W" -and (-not($BiosVersion -eq 'N21 Ver. 02.45')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP EliteDesk 800 65W G2 2.45 Rev.A\HPBIOSUPDREC\HPBIOSUPDREC64.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "HP EliteDesk 800 G3 SFF" -and (-not($BiosVersion -eq 'P01 Ver. 02.32')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP EliteDesk 800 G3 SFF 02.32 Rev.A\HPBIOSUPDREC\HPBIOSUPDREC64.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "HP ProBook 650 G2" -and (-not($BiosVersion -eq 'N76 Ver. 01.51' -or $BiosVersion -eq 'N87 Ver. 01.51')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP ProBook 650 G2 1.51 Rev.A\HPBIOSUPDREC64.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "HP ProBook 650 G4" -and (-not($BiosVersion -eq 'Q83 Ver. 01.10.00' -or $BiosVersion -eq 'Q77 Ver. 01.10.00')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP ProBook 650 G4 01.10.00 Rev.A\HpFirmwareUpdRec64.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "HP ProDesk 400 G5 SFF" -and (-not($BiosVersion -eq '02.10.00')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP ProDesk 400 G5 SFF 02.10.00 Rev.A\HpFirmwareUpdRec64.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "HP ZBook 17 G2" -and (-not($BiosVersion -eq 'M70 Ver. 01.25')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP ZBook 17 G2 1.25 Rev.A\HPBIOSUPDREC64.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "HP ZBook 17 G3" -and (-not($BiosVersion -eq 'N81 Ver. 01.45')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP ZBook 17 G3 1.45 Rev.A\HPBIOSUPDREC64.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "HP ZBook 17 G4" -and (-not($BiosVersion -eq 'P70 Ver. 01.32')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP ZBook 17 G4 1.32 Rev.A\HPBIOSUPDREC64.exe" -ArgumentList '-r -s' -Wait
...
if ( $pcModel -eq "HP ZBook 17 G5" -and (-not($BiosVersion -eq 'Q70 Ver. 01.10.01')) ) {
	...
	# Update BIOS
	Start-Process "\\frigga01.hbe.local\sources$\Software\BIOS Update\HP ZBook 17 G5 01.10.01 Rev.A\HpFirmwareUpdRec64.exe" -ArgumentList '-r -s' -Wait
