...
[CmdletBinding()] 
 Param  
   ( 
    [Alias("d")] 
    [String]$dir = "\\hewbefs001.hbe.local\ssc$\05 _IT\Audit\",  
    [String]$reportname = "reportfile.txt", 
    [Alias("r")] 
    [String]$Root = 'CN=Users,DC=HBE,DC=local', 
    [Alias("g")]  
    [String]$DomainGroup = "Domain Admins", 
    [String]$PreferredDC = "ODIN01.hbe.local" 
   )#End Param 
...
if ($difference) 
    { 
     ...
     Send-HTMLEmail -InputObject $DifferenceObject -Subject "$DomainGroup - Change in Group Members!" -To itbis@besix.com
    } 
 else 
    { 
