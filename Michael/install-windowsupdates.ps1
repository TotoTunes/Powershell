...
# Example command: PowerShell "C:\users\Administrator\Documents\install-windowsupdates.ps1" -computer MGT01
#>
...
ForEach ($c in $computer){
    ...
    $nugetinstall = invoke-command -ComputerName $c -ScriptBlock {[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force}
    invoke-command -ComputerName $c -ScriptBlock {[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; install-module pswindowsupdate -force}
    invoke-command -ComputerName $c -ScriptBlock {Set-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\Installer" -Name "DisableMSI" -Value 0;Set-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\Installer" -Name "DisablePatch" -Value 0}
	...
	invoke-command -ComputerName $c -ScriptBlock {gpupdate /force}
}